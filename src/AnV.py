...
def start():  # start is a function that check or make the file structure for the whole application
    ...
    projectList = listdir(
        pathData + "/USERDATA/")  # list of all "project" from past session - for each project one directory in USERDATA/
    ...
    for s in sampleList:  # sample = [sampleName, fastq1Path, fastq2Path, yAdd + 20, minion.get()]
        ...
        try:
            ...
            tk.messagebox.showinfo("Error..",
                                   "Error during mapping.. sample:" + str(s[0]) + "\n files may be corrupted!")
            sampleList.remove(s)
...
def map2sam(pathToFastq, genome_ref_covered, a, nameS):
    ...
    for name, seq, qual in generator:
        for hit in a.map(seq):
            ...
            if hit.strand == 1:  # orientation sequence +1
                ...
                fichierSAM1.write(
                    name + "\t0\t" + hit.ctg + "\t" + str(hit.r_st + 1) + "\t" + str(hit.mapq) + "\t" + str(
                        hit.cigar_str) + "\t" + "*" + "\t" + "0" + "\t" + str(hit.blen) + "\t" + seq[
                                                                                                 hit.q_st:hit.q_en] + "\t" + phred[
                                                                                                                             hit.q_st:hit.q_en] + "\n")
            if hit.strand == -1:
                # complement strand
    ...
    fichierSAM = open(pathData + "/USERDATA/" + projectSelected + "/" + nameS + "/" + nameS + ".sam",
...
def mapping(pathToFastq, db, nameS, type, pathToFastq2):
    ...
    for accResultSp in resultSp:
        fichierCSV.write(nameS + ", " + accResultSp + ", " + str(genome_ref_covered[accResultSp][1]) + ", " + str(
            genome_ref_covered[accResultSp][2]) + ", " + str(
            genome_ref_covered[accResultSp][3]) + ", " + str(genome_ref_covered[accResultSp][4]) + ", " + str(
            genome_ref_covered[accResultSp][5]) + ", " + genome_ref_class[accResultSp][0].split("=")[1] + ", " +
                         genome_ref_class[accResultSp][1].split("=")[1] + ", " +
                         genome_ref_class[accResultSp][2].split("=")[1] + ", " +
                         genome_ref_class[accResultSp][3].split("=")[1] + ", " +
                         genome_ref_class[accResultSp][4].split("=")[1] + "\n")
    fichierCSV.close()
    pb['value'] += 4
    main.update()
...
def file_save():
    ...
    for s in sampleList:
        try:
            fichier = open(pathData + "/USERDATA/" + projectSelected + "/" + s[0] + "/species.csv",
                           "r")  # if error during mapping file will not existe
...
def fasta_save():
    ...
    for s in sampleList:
        ...
        try:
            fichier = open(pathData + "/USERDATA/" + projectSelected + "/" + s[0] + "/species.csv",
                           "r")  # if error during mapping file will not exist
            for lane in fichier:
                try:
                    ac = lane.split(",")[1].split(" ")[1]

                    headerDico[ac] = lane.split(",")[9].split(" ")[1] + "_" + lane.split(",")[0] + "_" + \
                                     lane.split(",")[7].split(" ")[1]
                except:
                    pass
            fichier.close()
...
def grid():
    ...
    if fileList == []:
        ...
        for genus in genusNameList:
            ...
            listLenVirusByGenus.append(
                len(virusNameList))  # to get the max number of virus in a grid for later set the height of the graph
            for file in fileList:
                virusNameDico = {}
            ...
            fig.add_trace(go.Heatmap(z=data, name=str(genus),
                                     colorscale=[[0, 'whitesmoke'], [0.33, '#fad146'], [0.66, 'orange'],
                                                 [1, 'crimson']],
...
def placeAlpha():
    imgAlpha = tk.PhotoImage(file=pathData + "/IMG/TREE/ALPHA.png")
...
def placeBeta():
    imgBeta = tk.PhotoImage(file=pathData + "/IMG/TREE/BETA.png")
...
def placeGamma():
    imgGamma = tk.PhotoImage(file=pathData + "/IMG/TREE/GAMMA.png")
